...
@Component({
  ...
  imports: [CommonModule, TranslateModule, MatPaginatorModule, FormsModule,
...
@Output() view = new EventEmitter<any | { data: any; column: MatTableColDef }>();
@Output() downloadPdf = new EventEmitter<any | { data: any; column: MatTableColDef }>();
...
filters: Event[] = [];
$viewInitiated: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
...
this.setDisplayedColumns();
    ...
    this.subs.push(
        combineLatest([this.$dataBuilt, this.$viewInitiated, of(true)]).subscribe((obj:any) => {
...
addColGrabbers() {
    var ths: HTMLElement[] = $(`table#${this.t_id} > thead > tr > th > div`).get();
    ...
    for (let i = 0; i < cols.length; i++) {
        ...
        if (this.isShipmentListingTable == true) {
            if (cols.filter((col) => col.ColMinWidth != null && col.ColMinWidth > 0).length == 0) {
...
buildColumns() {
    ...
    if (uf != null && uf.version == UserPreferenceVersions.table_col_pref_version) {
        var storedCols: MatTableColDef[] = uf.preference_obj;
        if (storedCols.length != this._columns.length) {
            cols_changed = true;
        }
        for (let e of storedCols) {
            var existedCol = this._columns.find((a) => a.PropertyName == e.PropertyName);
            if (existedCol == null) {
                cols_changed = true;
                continue;
    ...
    if (uf != null && cols_changed == false && uf.version == UserPreferenceVersions.table_col_pref_version) {
...
assignColFunctions(cols: MatTableColDef[], rCols: MatTableColDef[]) {
    cols.forEach((col) => {
        ...
        if (col.ExpandableSubColModel && col.ExpandableSubColModel.length) {
            this.assignColFunctions(col.ExpandableSubColModel, _col.ExpandableSubColModel);
        }
    });
}
...
copyColumns(cols: MatTableColDef[]) {
    if (this.all_columns && this.all_columns.length > 0) {
        cols.forEach((c) => {
            var ec:any = this.all_columns.find((a) => a.PropertyName == c.PropertyName);
            if (ec != null) {
                ec.mapServiceObj(c);
            }
        });
...
getUserPreferences(): UserPreference | any {
    try {
        ...
        if (string_obj == null) {
            string_obj = CommonTableConfigBC.getInstance().getByIdentifier(this.type);
        }
...
viewData(data: any, column: MatTableColDef | any = null): void {
    if (this.emitWithRow) {
        this.view.emit({ data: data, column: column });
    } else {
        this.view.emit(data);
    }
}
...
showColEditDialog() {
    var dialogRef = this.dialog.open(EditTableColumnSelectionComponent, { data: this.all_columns });
    ...
    try {
        ...
        if (this.enableDefaultExcel == false) {
            ...
            this.all_columns
                ...
                .forEach((col) => {
                    columns.push({ header: col.Title, key: col.PropertyName, width: 32 });
                });
                ...
                columns.forEach((col:any) => {
                    row[col.key] =
                        d[col.key] && d[col.key].length > this.cellMaxChar
                            ? $('<div>').append(this.getText(d, col.key).substring(0, this.cellMaxChar)).text()
            ...
            sheet1.getRow(1).eachCell({ includeEmpty: true }, (cell:any, colNum:any) => {
                cell.font = {
                    color: { argb: 'ffffffff' },
                    bold: true,
                    size: 10,
...
async getBase64ImageFromUrl(imageUrl: string): Promise<string | ArrayBuffer | null> {
...
getChildContent(e: { data: any[]; columns: MatTableColDef[]; pageSize: PageSize }): {
    ...
    try {
        ...
        if (this.enableDefaultPDF == false) {
            ...
            if (result) {
                ...
                skipHeader = result.skipHeader != undefined? result.skipHeader: false;
            }
        } else {
            docDef = {
          ...
          if (this.isReport && this.fromDate != null && this.toDate != null && !this.isUnbilledShipmentReport) {
            ...
            if (Standard.getStrWithoutWhiteSpace(dateRangeText).length > 0) {
                content.push({ marginBottom: 5, text: dateRangeText, alignment: 'right', fontSize: 8 });
            }

            var titles = [];
            var inSummaryTable = false;
            if (this.all_columns[0].ExpandableSubColModel && this.all_columns[0].ExpandableSubColModel.length) {
                inSummaryTable = true;
            }
            if (inSummaryTable) {
                this.dataSource.filteredData.forEach((d) => {
                    ...
                    var string_obj = CommonTableConfigBC.getInstance().getByIdentifier(this.type + '_sub');
                    if (string_obj) {
                        var actual_obj: UserPreference = JSON.parse(string_obj);
                        child_all_columns = actual_obj.preference_obj;
                    }

                    var childT: { content: ContentTable; tableLayout: { key: string; layout: TableLayout } } | any =
        ...
        if (isHeaderImg) {
            if (headerLogo && headerLogo.ServerFilePath.length > 0) {
                _imgData = <string>await this.getBase64ImageFromUrl(headerLogo.ServerFilePath);
                if (_imgData) {
                    headerCol = {
                        margin: [20, 20, 0, 0],
                        image: _imgData,
            ...
            this.headerHtmlContent = this.sanitizer.bypassSecurityTrustHtml(headerContent);
        ...
        if (isFooterImg) {
            if (footerLogo && footerLogo.ServerFilePath.length > 0) {
                _imgData = <string>await this.getBase64ImageFromUrl(footerLogo.ServerFilePath);
                if (_imgData) {
                    footerCol = {
                        margin: [20, 10, 20, 0],
                        image: _imgData,
            ...
            this.footerHtmlContent = this.sanitizer.bypassSecurityTrustHtml(footerContent);
        ...
        var printedOnText = `Printed on ${Standard.getFormattedDateDisplay(new Date())} by -}`;
        ...
        pdfMake.createPdf(docDef, tableLayoutsObj, PdfMakerService.getInstance().fonts).download(this.title);
    } catch (ex) {
        console.error(ex);
...
getPageSize(): PageSize {
    ...
    if (this.all_columns[0].ExpandableSubColModel != null && this.all_columns[0].ExpandableSubColModel.length > 0) {
        var stringObj = CommonTableConfigBC.getInstance().getByIdentifier(this.type + '_sub');
        if (stringObj) {
        ...
        } else if (this.all_columns[0].ExpandableSubColModel.filter((a:any) => a.IsSelected).length > len) {
            len = this.all_columns[0].ExpandableSubColModel.filter((a:any) => a.IsSelected).length;
        }
...
async getHeaderOrFooterContentAsImg(): Promise<{ img: string; height: number } | null> {
    var _p = new Promise<{ img: string; height: number }|null>((res, rej) => {
        setTimeout(() => {
            try {
                var _h: number = 0;
                var node: HTMLElement = <HTMLElement>document.getElementById(this.printSectionId);
                if (node) {
                    var innerChild = node;
                    _h = +($(innerChild).prop("scrollHeight")?.toString().replace('px', ''));
                }
...
public buildColData(): void {
    this.all_columns.forEach((alCal: any) => {
        ...
        try {
            if (data && data.length) {
                if (!/^\d+$/.test(data[0]) && !isNaN(new Date(this.sanitizeDateForFilter(data[0])).getDate())) {
...
onChange(column: MatTableColDef) {
            ...
            _data = _data.filter(
                (d) =>
                    ...
                    (alCal.ColFilterDataSource != null &&
                        intersection(d[alCal.ColFilterDataSource], alCal.filterdata).length > 0)
            );
        }
    });
    ...
    this.all_columns.forEach((alCal) => {
        ...
        if (alCal.Title != column.Title || (alCal.Title == column.Title && !column.filterdata?.length)) {
            alCal.colFilterData = data;
        }
    });
}
...
toggleClose(column: MatTableColDef): void {
    var ec: any = this.all_columns.find((a) => a.PropertyName == column.PropertyName);
...
footerCountForPriceReport(allData: any[], propertyName: string) {
    ...
    const data = allData.filter((u, i) => i >= skip).filter((u, i) => i < this._paginator.pageSize);

    switch (propertyName) {
        case 'BranchName':
            return 'Total';
        case 'SearchCount':
            return data.map((x) => x.SearchCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'InquiryCount':
            return data.map((x) => x.InquiryCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'QuotationCount':
            return data.map((x) => x.QuotationCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'BookingsCount':
            return data.map((x) => x.BookingsCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'UsersCount':
            return data.map((x) => x.UsersCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'RateFilesProcessedCount':
            return data.map((x) => x.RateFilesProcessedCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'UsedCustomerPortalUsersCount':
            return data.map((x) => x.UsedCustomerPortalUsersCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'ConfirmQuotationAPIUsageCount':
            return data.map((x) => x.ConfirmQuotationAPIUsageCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'PortalCustomersCount':
            return data.map((x) => x.PortalCustomersCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'InquiriesAPIUsageCount':
            return data.map((x) => x.InquiriesAPIUsageCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'InquiriesManualUsageCount':
            return data.map((x) => x.InquiriesManualUsageCount).reduce((partialSum, a) => partialSum + a, 0);
        case 'InquiriesPortalUsageCount':
            return data.map((x) => x.InquiriesPortalUsageCount).reduce((partialSum, a) => partialSum + a, 0);
        default:
            return '';
    }
}
